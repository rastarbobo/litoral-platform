...
async function handleCustomEdge(
  ...
  if (request.method === "POST" && url.pathname === "/api/webhooks/tracking") {
    try {
      ...
      const body = await request.json() as { prospectId?: string; eventType?: string; metadata?: Record<string, any> };
  ...
  const worker = {
    ...
    async queue(batch: MessageBatch<any>, __env: Env, __ctx: ExecutionContext): Promise<void> {
